...
export function VerifyPaymentTable(props: DataTableProps) {
  const { data, isLoading, onPageChange, currentPage, totalPages } = props;

  const columns: ColumnDef<VerifyPaymentData>[] = [
    {
      accessorKey: "verifiedDate",
      header: () => <div className="font-medium">Verified Date</div>,
      cell: ({ row }) => {
        const date = row.getValue("verifiedDate") as string;
        return <div>{date}</div>;
      },
    },
    {
    ...
    },
    {
      ...
      header: () => <div className="font-medium">Customer Name</div>,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => <div className="font-medium">Verified By</div>,
      cell: ({ row }) => (
        <div>{row.getValue("verifiedBy")}</div>
      ),
    },
  ];
