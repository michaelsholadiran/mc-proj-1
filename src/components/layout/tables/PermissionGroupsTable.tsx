...
interface DataTableProps {
  ...
  onEditTableProp: (permissionGroup: PermissionGroup) => void;
}

export function PermissionGroupsTable(props: DataTableProps) {
  const { data, isLoading, onDeleteTableProp, onEditTableProp } = props;
  const columns: ColumnDef<PermissionGroup>[] = [
    {
      id: "group",
      header: () => <div className="p-[10px]">Permission Group</div>,
      cell: ({ row }) => {
        const permissionGroup = row.original;

        return (
          <div className="capitalize p-[10px]">
            <Link href={`/permission-management/${permissionGroup?.groupId}`}>
